...
describe("app-composer knobs ported from the glow field", () => {
  it("orc-textarea takes rows, description and exposes a public textarea", () => {
    const field = document.createElement("orc-textarea");
  ...
  it("orc-input takes the same description contract and exposes its input", () => {
    const field = document.createElement("orc-input");
    field.setAttribute("description", "Lowercase, no spaces");
    document.body.append(field);

    const input = field.shadowRoot!.querySelector("input")!;
    expect((field as { input?: HTMLInputElement }).input).toBe(input);

    const description = field.shadowRoot!.querySelector<HTMLElement>(
      ".description",
    )!;
    expect(input.getAttribute("aria-describedby")).toBe(description.id);
    expect(description.textContent).toBe("Lowercase, no spaces");
  ...
  it("quickens the beam on focus — the focus cue that is not carried by colour", () => {
    for (const [tag, control] of [
      ...
      expect(css).toContain(`.field:has(${control}:focus)::before { animation-duration: 3s; }`);
    }
  });
});
